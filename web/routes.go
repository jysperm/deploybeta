...
func CreateWebServer() *echo.Echo {
	...
	app.POST("/apps/:name/versions", handlers.CreateVersion, helpers.AuthenticateMiddleware)
	app.POST("/apps/:name/version", handlers.CreateAndDeploy, helpers.AuthenticateMiddleware)
	app.PUT("/apps/:name/version", handlers.DeployVersion, helpers.AuthenticateMiddleware)
